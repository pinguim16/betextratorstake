...
function stake() {
    ...
    chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
      if (request.action === 'extractBets') {
        ...
        try {
          const data = await extractStakeBets(startDate, endDate);
          if (data.length > 0) {
            downloadCSV(data);
    ...
    // Função para extrair as apostas do Stake dentro do intervalo de datas
    async function extractStakeBets(startDate, endDate) {
    ...
    // Função para extrair apostas da página atual
    ...
    // Função para converter a data do formato do site para o formato necessário
    ...
    // Função para extrair os dados de uma aposta individual
    function parseBetData(betElement, betDate) {
      const stateElement = betElement.querySelector('div.badge');
      let state = 'Unknown';
  
      if (stateElement) {
        state = stateElement.classList.contains('variant-success') ? 'W' : 'L';
      }
      ...
      const stake = stakeElement ? stakeElement.innerText.trim().replace('R$', '').replace(',', '.').trim() : '0.00';
      ...
      const odds = oddsElement ? oddsElement.innerText.trim().replace(',', '.').trim() : '0.00';
    ...
    // Função para navegar para a próxima página de apostas
